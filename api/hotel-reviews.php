...
switch ($action) {
    ...
    case 'review':
        ...
        $stayed   = trim((string) input('stayed_on',  'post', ''));
        ...
        $photoErrors = save_review_photos('hotel', $newReviewId);
    ...
    case 'update_review':
        ...
        $existing = db_fetch_one("SELECT * FROM hotel_reviews WHERE id = ?", [$reviewId]);
        ...
        db_execute(
            "UPDATE hotel_reviews SET rating=?, title=?, body=?, stayed_on=? WHERE id=? AND user_id=?",
            [$rating, $title ?: null, $body ?: null, ($stayed ?: null), $reviewId, $user_id]
        );

        $photoErrors = save_review_photos('hotel', $reviewId);
    ...
    case 'delete_review':
        ...
        db_execute("DELETE FROM hotel_reviews WHERE id=? AND user_id=?", [$reviewId, $user_id]);
        db_execute("DELETE FROM review_photos WHERE review_type='hotel' AND review_id=?", [$reviewId]);
